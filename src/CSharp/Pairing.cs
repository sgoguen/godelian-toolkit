...
namespace GodalToolkitFSharp.Math;
...
public static class BigIntegerExtensions {
    public static BigInteger Sqrt(this BigInteger z) {
        if (z < 0) {
            throw new ArgumentException("Cannot compute the square root of a negative number", nameof(z));
        } else if (z == 0) {
            return 0;
        } else {
            BigInteger NewtonRaphson(BigInteger x) {
                BigInteger NextX() => (x + z / x) / 2;
...
public static class Pairing {
    public static (BigInteger, BigInteger) EncodePair(BigInteger z) {
        BigInteger M = z.Sqrt();
        BigInteger M2 = M * M;
        return z - M2 < M ? (z - M2, M) : (M, M2 + 2 * M - z);
    }

    public static BigInteger DecodePair((BigInteger, BigInteger) p) {
        (BigInteger x, BigInteger y) = p;
        BigInteger M = BigInteger.Max(x, y);
        return M * M + M + x - y;
    }
}
